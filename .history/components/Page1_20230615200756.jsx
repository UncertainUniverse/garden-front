...
const Page1 = forwardRef((props, ref) => {
	...
	function cleanUp(mesh) {
		console.log(mesh)
		// mesh.current.mesh.material.map.dispose()
		// mesh.current.mesh.geometry.dispose()
		// mesh.current.mesh.material.dispose()
		// scene.remove(mesh)
	}
		...
		const handleWheel = (event) => {
			...
			if (!animRef.current) {
				if (event.deltaY >= 25) {
					...
					gsap.to(pRef.current.position, { z: -13, duration: 3 })
